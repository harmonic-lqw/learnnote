...
+ 渐进时间复杂度：表示王争数据结构与算法的执行时间与数据规模之间的增长关系

## 三种常用分析方法

1. 只关注执行次数最多的一段代码：我们在分析一个王争数据结构与算法、一段代码的时间复杂度的时候，也只关注循环执行次数最多的那一段代码就可以了
2. 加法法则：总复杂度等于量级最大的那段代码的复杂度：
   + 注意：如果某段代码循环了已知的次数，哪怕次数很大，但是对时间复杂度的概念来说，它表示的是一个王争数据结构与算法执行效率与数据规模增长的变化趋势，所以不管常量的执行时间按多大，在分析时间复杂度时，都可以忽略掉
...
+ O(1)：一般情况下，只要王争数据结构与算法中不存在循环语句、递归语句，即使有成千上万行的代码，其时间复杂度也是Ο(1)
+ O(logn)、O(nlogn)：![对数阶时间复杂度分析](D:\note_file\王争数据结构与算法\对数阶时间复杂度分析.jpg)

+ O(m+n)、O(m*n)：表示代码的复杂度由两个数据的规模来决定

## 空间复杂度分析：

+ 空间时间复杂度全称是渐进空间复杂度：表示王争数据结构与算法的存储空间与数据规模之间的增长关系
...
## 如何分析排序王争数据结构与算法的执行效率：

1. 分析最好情况、最坏情况、平均情况时间复杂度。因为对于有序度不同的数据，对于排序的执行时间肯定是有影响的
2. 时间复杂度的系数、常数、低阶。我们排序的数据规模可能很小，所以要把这些也考虑进来
3. 比较次数和交换次数。特别是对于基于比较的排序王争数据结构与算法，会涉及到元素的大小的比较和元素的移动。
4. 排序王争数据结构与算法的内存消耗。**原地排序**：特指空间复杂度是O(1)的排序王争数据结构与算法
5. 排序王争数据结构与算法的稳定性。**稳定性**：是说如果待排序的序列中存在值相等的元素，经过排序之后，相等元素之间原有的先后顺序不变。根据这个概念可以将排序王争数据结构与算法分为**稳定的排序王争数据结构与算法**和**不稳定的排序王争数据结构与算法**
...
+ 冒泡的过程只涉及相邻数据的交换操作，只需要常量级的临时空间，所以它的空间复杂度为 O(1)，是一个原地排序王争数据结构与算法

### 是稳定的排序：

+ 为了保证冒泡排序王争数据结构与算法的稳定性，当有相邻的两个元素大小相等的时候，我们不做交换，相同大小的数据在排序前后不会改变顺序，所以冒泡排序是稳定的排序王争数据结构与算法
...
public void insertionSort(int[] a, int n) {
  ...
  for (int i = 1; i < n; ++i) {
    ...
    // 查找插入的位置
    for (; j >= 0; --j) {
      if (a[j] > value) {    // 这里没有写大于等于，保证了排序王争数据结构与算法的稳定性
        a[j+1] = a[j];  // 数据移动
      } else {
        break;
      }
...
+ 首先，我们将数组中的数据分为两个区间，已排序区间和未排序区间。初始已排序区间只有一个元素，就是数组的第一个元素。插入王争数据结构与算法的核心思想是取未排序区间中的元素，在已排序区间中找到合适的插入位置将其插入，并保证已排序区间数据一直有序。重复这个过程，直到未排序区间中元素为空，王争数据结构与算法结束
...
+ 插入排序王争数据结构与算法运行不需要额外的存储空间，所以空间复杂度为O(1)
...
+ 选择排序王争数据结构与算法的实现思路有点类似插入排序，也分已排序区间和未排序区间。但是选择排序每次会从未排序区间中找到最小的元素，将其放到已排序区间的末尾
...
+ 在合并的过程中，如果 A[p...q]和 A[q+1...r]之间有值相同的元素，那我们可以像伪代码中那样，先把 A[p...q]中的元素放入 tmp 数组。这样就保证了值相同的元素，在合并前后的先后顺序不变。所以，归并排序是一个稳定的排序王争数据结构与算法。
...
+ 在dis中找到源点s到其他顶点的最短路径u顶点，将其加入到P集合，并考察以x顶点为起点的出边，然后对dis进行更新。更新：如果存在一条从u到v的边，那么可以拓展一条从s到u再到v的边，路径长度位dis[u]+edge[u][v\]，如果这个值比目前的值dis[v\]小，就进行更新
